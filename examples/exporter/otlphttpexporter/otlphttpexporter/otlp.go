...
package otlphttpexporter // import "github.com/otelwasm/otelwasm/examples/exporter/otlphttpexporter"
...
func newExporter(cfg component.Config, set exporter.Settings) (*baseExporter, error) {
	println("Creating new exporter", cfg)
	oCfg := cfg.(*Config)
...
func (e *baseExporter) start(ctx context.Context, host component.Host) error {
	println("Starting HTTP client", e.config.ClientConfig.Endpoint)

	client, err := e.config.ClientConfig.ToClient(ctx, host, e.settings)
	if err != nil {
		println("Failed to create HTTP client", e.config.ClientConfig.Endpoint, err)
		return err
	}

	println("Starting HTTP client", e.config.ClientConfig.Endpoint, client)
...
func (e *baseExporter) pushTraces(ctx context.Context, td ptrace.Traces) error {
	println("Called pushTraces", e.config.ClientConfig.Endpoint)

	tr := ptraceotlp.NewExportRequestFromTraces(td)

	println("Pushing traces", e.config.ClientConfig.Endpoint)
	println("Pushing traces", e.tracesURL)
	println("Pushing traces", e.config.Encoding)
	println("Pushing traces", e.config.ClientConfig.Headers)
...
func (e *baseExporter) export(ctx context.Context, url string, request []byte, partialSuccessHandler partialSuccessHandler) error {
	...
	println("Preparing to make HTTP request", url)
