...
guest's `alloc()` function is owned and freed by the guest. This follows the established
...
1. `alloc(size)` — allocate memory in guest
...
validate that required functions (`alloc`, `start`, `shutdown`, etc.) are actually
implemented.
...
| **otelwasm v1** | `alloc` | None | Guest-managed |
...
2. **Ownership semantics**: The guest calls `alloc`, so the guest owns the memory. The host
   merely writes to it. Ownership transfer would complicate the ABI and create GC issues.

3. **Single-threaded safety**: WASM execution is single-threaded. Between `alloc()` and
