...
- [12. WASI Support](#12-wasi-support)
...
| `ERROR`   | `1`   | Operation failed; reason SHOULD be provided via `otelwasm_set_status_reason` |
...
```
otelwasm_memory_allocate(size: i32) -> i32
```
...
The host calls `otelwasm_memory_allocate` when it needs to write data into guest memory (e.g., before calling
`otelwasm_consume_traces` with telemetry data). The guest is responsible for freeing the allocated
...
2. Host calls `otelwasm_memory_allocate(data_size)` to allocate memory in the guest.
3. Host writes the serialized data into the allocated region.
4. Host calls the guest's consumer function (e.g., `otelwasm_consume_traces(data_ptr, data_size)`).
...
```
otelwasm_abi_version_0_1_0() -> void
```
...
1. Check for `otelwasm_abi_version_0_1_0` export → use ABI v1 semantics.
...
Future ABI versions will use markers named `otelwasm_abi_version_0_2_0`, `otelwasm_abi_version_0_3_0`, etc. A module
...
Module Load
  → ABI version detection
    → otelwasm_start()                             ← component.Start(ctx, host)
      → [Component operations]
        → otelwasm_shutdown()                      ← component.Shutdown(ctx)
          → Module unload
```
...
### 6.4 Start (`otelwasm_start`)

The host calls `otelwasm_start()`. This maps to `component.Component.Start(ctx, host)`.

During this phase, the guest MAY:

- Call `otelwasm_get_plugin_config()` to retrieve and validate configuration.
- Initialize internal state and resources.
- Call `otelwasm_log()` to emit diagnostic messages.
...
`otelwasm_set_status_reason()`.

The `otelwasm_start()` function MUST return promptly. Long-running initialization is discouraged.

### 6.5 Component Operations

Depending on the component type:

- **Processors**: The host calls `otelwasm_consume_traces`, `otelwasm_consume_metrics`, or `otelwasm_consume_logs` for
  each batch of telemetry data passing through the pipeline. The guest processes the data and
  calls `otelwasm_set_result_*()` to forward results to the next pipeline stage.
- **Exporters**: The host calls `otelwasm_consume_traces`, `otelwasm_consume_metrics`, or `otelwasm_consume_logs` for
  each batch of telemetry data to be exported. The guest handles the export operation and
  returns a status.
- **Receivers**: The host calls `otelwasm_start_traces_receiver`, `otelwasm_start_metrics_receiver`, or
  `otelwasm_start_logs_receiver` once. The function blocks until shutdown is requested, emitting
  telemetry via `otelwasm_set_result_*()`.

### 6.6 Shutdown (`otelwasm_shutdown`)

This maps to `component.Component.Shutdown(ctx)`.

When the OpenTelemetry Collector is shutting down:

1. For receivers: the host sets the shutdown flag (readable via `otelwasm_get_shutdown_requested()`),
   causing the receiver loop to exit.
2. The host waits for any in-flight operations to complete.
3. The host calls `otelwasm_shutdown()`.
...
#### `otelwasm_set_result_traces`

```
otelwasm_set_result_traces(data_ptr: i32, data_size: i32) -> void
```
...
**Valid context**: During `otelwasm_consume_traces` and `otelwasm_start_traces_receiver` invocations.

#### `otelwasm_set_result_metrics`

```
otelwasm_set_result_metrics(data_ptr: i32, data_size: i32) -> void
```

Submits metrics data to the next consumer in the pipeline.

**Valid context**: During `otelwasm_consume_metrics` and `otelwasm_start_metrics_receiver` invocations.

#### `otelwasm_set_result_logs`

```
otelwasm_set_result_logs(data_ptr: i32, data_size: i32) -> void
```

Submits logs data to the next consumer in the pipeline.

**Valid context**: During `otelwasm_consume_logs` and `otelwasm_start_logs_receiver` invocations.

### 7.2 Configuration

#### `otelwasm_get_plugin_config`

```
otelwasm_get_plugin_config(buf_ptr: i32, buf_limit: i32) -> i32
```
...
```
otelwasm_set_status_reason(msg_ptr: i32, msg_size: i32) -> void
```
...
#### `otelwasm_get_shutdown_requested`

```
otelwasm_get_shutdown_requested() -> i32
```
...
```
otelwasm_log(level: i32, msg_ptr: i32, msg_size: i32) -> void
```
...
#### `otelwasm_get_supported_telemetry`

```
otelwasm_get_supported_telemetry() -> i32
```
...
```
otelwasm_shutdown() -> i32
```

Called once to shut down the component. Maps to `component.Component.Shutdown(ctx)` (see
§6.6). Returns a `StatusCode` (§3.1). MUST be safe to call even if `otelwasm_start()` was not called
...
corresponding to the telemetry types declared by `otelwasm_get_supported_telemetry()`.

#### `otelwasm_consume_traces`

```
otelwasm_consume_traces(data_ptr: i32, data_size: i32) -> i32
```
...
- `data_ptr` — Pointer to OTLP protobuf-encoded traces data in guest memory (allocated by the
  host via `otelwasm_memory_allocate`).
...
3. Call `otelwasm_set_result_traces()` to forward the result to the next consumer.
4. Return a `StatusCode`.

If `otelwasm_set_result_traces()` is not called and the function returns `SUCCESS`, the host SHOULD
...
```
otelwasm_consume_metrics(data_ptr: i32, data_size: i32) -> i32
```

Consume a batch of metrics. Maps to `consumer.ConsumeMetrics(ctx, pmetric.Metrics)`.
Same protocol as `otelwasm_consume_traces`.

#### `otelwasm_consume_logs`

```
otelwasm_consume_logs(data_ptr: i32, data_size: i32) -> i32
```
...
```
otelwasm_start_traces_receiver() -> void
```
...
3. Call `otelwasm_set_result_traces()` to emit data into the pipeline (acting as the next consumer).
4. Check `otelwasm_get_shutdown_requested()` periodically.
5. Exit the loop when shutdown is indicated.

#### `otelwasm_start_metrics_receiver`

```
otelwasm_start_metrics_receiver() -> void
```

Start the metrics receiver. Same protocol as `otelwasm_start_traces_receiver`.

#### `otelwasm_start_logs_receiver`

```
otelwasm_start_logs_receiver() -> void
```

Start the logs receiver. Same protocol as `otelwasm_start_traces_receiver`.
...
1. The guest SHOULD call `otelwasm_set_status_reason()` before returning to provide a human-readable
   error description.
...
If `otelwasm_memory_allocate` returns `0` (allocation failure), the host SHOULD:
...
This section documents known limitations and design trade-offs in ABI v1. These are
intentional design decisions made to balance simplicity, performance, and cross-language
compatibility.

### 11.1 Memory Management Complexity

**Go Language GC Considerations:**

The push model requires Go guest implementations to use a pinning map to prevent garbage
collection of allocated buffers (see [design-notes-push-model.md](design-notes-push-model.md)
§3). While this adds complexity, it is a well-understood pattern and has minimal runtime
overhead.

**No `dealloc` Export:**

The ABI intentionally omits a `dealloc` or `free` host function. Memory allocated by the
guest's `otelwasm_memory_allocate()` function is owned and freed by the guest. This follows the established
pattern in other WASM ABIs (proxy-wasm, http-wasm) and provides:

- **Guest autonomy**: Each language manages memory according to its own conventions (GC,
  RAII, manual)
- **Ownership clarity**: Guest allocates → guest deallocates
- **Simplicity**: Fewer host functions, clearer semantics

See Appendix D for rationale and comparison with other WASM ABIs.

### 11.2 Context Propagation

Go's `context.Context` is not propagated to WASM guest functions. This means:

- **No deadline/timeout propagation**: Guests cannot observe or respect timeouts set in the
  Collector's context
- **No cancellation signals**: Context cancellation does not reach the guest (except via
  `otelwasm_get_shutdown_requested()` for graceful shutdown)
- **No context values**: Trace IDs, correlation IDs, and other context-attached values are
  not accessible

**Workaround**: For graceful shutdown, receivers SHOULD poll `otelwasm_get_shutdown_requested()`. For
timeout-like behavior, guests can use WASI `clock_time_get` and implement their own timeout
logic.

### 11.3 Allocation Overhead

The push model requires two host-guest calls per telemetry batch:

1. `otelwasm_memory_allocate(size)` — allocate memory in guest
2. `otelwasm_consume_traces/metrics/logs(ptr, size)` — process data

This compares to one call in the experimental pull model. However:

- The overhead is minimal (nanoseconds per call in modern WASM runtimes)
- The push model's alignment with OTel Collector interfaces outweighs this cost
- Multi-language SDK support is significantly easier with the push model

### 11.4 Error Handling Gaps

**`otelwasm_set_status_reason` is optional:**

The spec uses SHOULD rather than MUST for calling `otelwasm_set_status_reason()` before returning an
error. This means error messages may be missing in some cases, making debugging harder.

**Rationale**: Enforcing MUST would be difficult without breaking existing plugins. Future
ABI versions may strengthen this requirement.

**Best practice**: Guest implementations SHOULD always call `otelwasm_set_status_reason()` before
returning a non-zero status code.

### 11.5 Network I/O Limitations

Guest modules cannot make outbound HTTP/gRPC requests. This limits exporter implementations
to:

- WASI stdout/stderr output
- Writing to files (via WASI)
- Calling `otelwasm_set_result_*()` to forward data to a native exporter

See Appendix D for future considerations around HTTP client host functions.

### 11.6 Backpressure Absence

There is no mechanism for the host to signal backpressure to receivers. A receiver emitting
data faster than the pipeline can process will buffer in the Collector's internal queues.

See Appendix D for future backpressure signaling mechanisms.

### 11.7 Type Safety Limitations

All data is passed as raw pointers and sizes (`i32`). There is no compile-time type checking
to ensure:

- Correct data format (OTLP protobuf)
- Correct telemetry type (traces vs metrics vs logs)
- Valid pointer addresses

**Mitigation**: Runtime validation and clear SDK documentation help prevent misuse.

**Future**: The [WebAssembly Component Model](https://github.com/WebAssembly/component-model)
provides strong typing for WASM interfaces, but is not yet widely adopted.

### 11.8 ABI Version Detection Fragility

The host detects ABI version by checking for the `otelwasm_abi_version_0_1_0` export. This does not
validate that required functions (`otelwasm_memory_allocate`, `otelwasm_start`, `otelwasm_shutdown`, etc.) are actually
implemented.

**Mitigation**: Hosts SHOULD validate required exports after detecting the ABI version and
fail gracefully with a clear error message if functions are missing.
...
> **Note**: Guest modules SHOULD prefer the `otelwasm_log` host function (§7.5) over WASI stdio for
...
|--------------------------|-----------------------------|------------------------------------|
| `otelwasm_set_result_traces`      | `(i32, i32) -> ()`         | Forward traces to next consumer    |
| `otelwasm_set_result_metrics`     | `(i32, i32) -> ()`         | Forward metrics to next consumer   |
| `otelwasm_set_result_logs`        | `(i32, i32) -> ()`         | Forward logs to next consumer      |
| `otelwasm_get_plugin_config`      | `(i32, i32) -> (i32)`      | Read plugin configuration (JSON)   |
| `otelwasm_set_status_reason`      | `(i32, i32) -> ()`         | Set error reason string            |
| `otelwasm_get_shutdown_requested` | `() -> (i32)`               | Check shutdown flag                |
| `otelwasm_log`           | `(i32, i32, i32) -> ()`    | Emit log message                   |

### Guest Exports

| Function                   | WASM Signature         | Required | Description                     |
|----------------------------|------------------------|----------|---------------------------------|
| `otelwasm_abi_version_0_1_0`           | `() -> ()`             | Yes      | ABI version marker              |
| `otelwasm_memory_allocate`                    | `(i32) -> (i32)`       | Yes      | Allocate guest memory for host  |
| `otelwasm_get_supported_telemetry`  | `() -> (i32)`          | Yes      | Supported telemetry bitmask     |
| `otelwasm_start`           | `() -> (i32)`          | Yes      | Start component                 |
| `otelwasm_shutdown`        | `() -> (i32)`          | Yes      | Shutdown component              |
| `otelwasm_consume_traces`           | `(i32, i32) -> (i32)`  | No\*     | Consume traces (proc/exp)       |
| `otelwasm_consume_metrics`          | `(i32, i32) -> (i32)`  | No\*     | Consume metrics (proc/exp)      |
| `otelwasm_consume_logs`             | `(i32, i32) -> (i32)`  | No\*     | Consume logs (proc/exp)         |
| `otelwasm_start_traces_receiver`    | `() -> ()`             | No\*     | Start traces receiver           |
| `otelwasm_start_metrics_receiver`   | `() -> ()`             | No\*     | Start metrics receiver          |
| `otelwasm_start_logs_receiver`      | `() -> ()`             | No\*     | Start logs receiver             |

\* Required based on the component type and the telemetry signals declared by
`otelwasm_get_supported_telemetry()`. Processors and exporters MUST export the appropriate `consume_*`
...
|-----------------------------------------|-----------------------|
| `component.Start(ctx, host) error`      | `otelwasm_start() -> i32`      |
| `component.Shutdown(ctx) error`         | `otelwasm_shutdown() -> i32`   |
...
| Go Interface                                          | ABI v1 Guest Export                      |
|-------------------------------------------------------|------------------------------------------|
| `consumer.ConsumeTraces(ctx, ptrace.Traces) error`    | `otelwasm_consume_traces(ptr, size) -> i32`       |
| `consumer.ConsumeMetrics(ctx, pmetric.Metrics) error` | `otelwasm_consume_metrics(ptr, size) -> i32`      |
| `consumer.ConsumeLogs(ctx, plog.Logs) error`          | `otelwasm_consume_logs(ptr, size) -> i32`         |
...
|---------------------------------------------------------|----------------------------------------|
| `nextConsumer.ConsumeTraces(ctx, td)`                   | `otelwasm_set_result_traces(ptr, size)`         |
| `nextConsumer.ConsumeMetrics(ctx, md)`                  | `otelwasm_set_result_metrics(ptr, size)`        |
| `nextConsumer.ConsumeLogs(ctx, ld)`                     | `otelwasm_set_result_logs(ptr, size)`           |

### Component Types

| OTel Collector Type    | Go Interface                               | ABI v1 Guest Exports                          |
|------------------------|--------------------------------------------|-----------------------------------------------|
| `processor.Traces`     | `component.Component + consumer.Traces`    | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_consume_traces`       |
| `processor.Metrics`    | `component.Component + consumer.Metrics`   | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_consume_metrics`      |
| `processor.Logs`       | `component.Component + consumer.Logs`      | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_consume_logs`         |
| `exporter.Traces`      | `component.Component + consumer.Traces`    | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_consume_traces`       |
| `exporter.Metrics`     | `component.Component + consumer.Metrics`   | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_consume_metrics`      |
| `exporter.Logs`        | `component.Component + consumer.Logs`      | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_consume_logs`         |
| `receiver.Traces`      | `component.Component`                      | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_start_traces_receiver`|
| `receiver.Metrics`     | `component.Component`                      | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_start_metrics_receiver`|
| `receiver.Logs`        | `component.Component`                      | `otelwasm_start` + `otelwasm_shutdown` + `otelwasm_start_logs_receiver`  |

### Factory and Configuration

| OTel Collector Pattern                                                  | ABI v1 Equivalent                          |
|-------------------------------------------------------------------------|--------------------------------------------|
| `factory.CreateDefaultConfig() component.Config`                        | `otelwasm_get_plugin_config()` host function        |
| `factory.CreateTraces(ctx, settings, config, next) (processor.Traces)`  | Module load + `otelwasm_start()` + `otelwasm_consume_traces` |
| `settings.ID`                                                           | Managed by host configuration              |
| `settings.TelemetrySettings` (logger)                                   | `otelwasm_log()` host function             |
...
| **Memory allocation** | Guest pre-allocates, two-pass retry       | Host allocates via guest's `otelwasm_memory_allocate()`      |
| **Proc/Exp interface**| Separate: `process_*()` / `push_*()`     | Unified: `consume_*()`                    |
| **Lifecycle**         | `plugin_init` / `plugin_shutdown`         | `otelwasm_start` / `otelwasm_shutdown`                      |
...
| `setResultTraces`                 | `otelwasm_set_result_traces`          | Renamed (snake\_case)               |
| `setResultMetrics`                | `otelwasm_set_result_metrics`         | Renamed                             |
| `setResultLogs`                   | `otelwasm_set_result_logs`            | Renamed                             |
| `getPluginConfig`                 | `otelwasm_get_plugin_config`          | Renamed                             |
| `setResultStatusReason`           | `otelwasm_set_status_reason`          | Renamed, shortened                  |
| `getShutdownRequested`            | `otelwasm_get_shutdown_requested`     | Renamed                             |
| `getSupportedTelemetry`           | `otelwasm_get_supported_telemetry`    | Renamed                             |
| `processTraces()`                 | `otelwasm_consume_traces(ptr, size)`  | Renamed, push model                 |
| `processMetrics()`                | `otelwasm_consume_metrics(ptr, size)` | Renamed, push model                 |
| `processLogs()`                   | `otelwasm_consume_logs(ptr, size)`    | Renamed, push model                 |
| `pushTraces()`                    | `otelwasm_consume_traces(ptr, size)`  | Unified with processor              |
| `pushMetrics()`                   | `otelwasm_consume_metrics(ptr, size)` | Unified with processor              |
| `pushLogs()`                      | `otelwasm_consume_logs(ptr, size)`    | Unified with processor              |
| `startTracesReceiver`             | `otelwasm_start_traces_receiver`      | Renamed                             |
| `startMetricsReceiver`            | `otelwasm_start_metrics_receiver`     | Renamed                             |
| `startLogsReceiver`               | `otelwasm_start_logs_receiver`        | Renamed                             |
| *(not present)*                   | `otelwasm_abi_version_0_1_0`             | **New**: ABI version marker         |
| *(not present)*                   | `otelwasm_memory_allocate`                      | **New**: Guest memory allocator     |
| *(not present)*                   | `otelwasm_start`             | **New**: Component start            |
| *(not present)*                   | `otelwasm_shutdown`          | **New**: Component shutdown         |
| *(not present)*                   | `otelwasm_log`               | **New**: Structured logging         |
...
1. On module load, check for `otelwasm_abi_version_0_1_0` export.
...
**Why there is no `dealloc` host function:**

The ABI intentionally omits a `dealloc(ptr, size)` or `free(ptr)` host function that would
allow the host to explicitly free guest memory. This follows the standard pattern in
WebAssembly ABIs where the guest owns its linear memory.

**Comparison with other WASM ABIs:**

| ABI Specification | Allocation Function | Deallocation Function | Pattern |
|-------------------|--------------------|-----------------------|---------|
| **proxy-wasm v0.2.1** | `proxy_on_memory_allocate` or `malloc` | None | Guest-managed |
| **http-wasm** | None (buffer-passing) | None | Guest-controlled |
| **Component Model** | `cabi_realloc` | `cabi_realloc(ptr, 0, 0, 0)` | Canonical ABI |
| **otelwasm v1** | `otelwasm_memory_allocate` | None | Guest-managed |

**Rationale for guest-managed deallocation:**

1. **Language flexibility**: Different languages have different memory management strategies:
   - Go: Garbage collection (automatic)
   - Rust: RAII with Drop trait (automatic)
   - Zig: Manual allocation/deallocation
   - A single `dealloc` function cannot accommodate all these models

2. **Ownership semantics**: The guest calls `otelwasm_memory_allocate`, so the guest owns the memory. The host
   merely writes to it. Ownership transfer would complicate the ABI and create GC issues.

3. **Single-threaded safety**: WASM execution is single-threaded. Between `otelwasm_memory_allocate()` and
   `consume_*()`, no guest code runs, so there is no GC risk. After `consume_*()` returns,
   the guest can safely deallocate.

4. **Industry precedent**: Proxy-wasm's widely-used ABI (implemented by Envoy, Istio, etc.)
   uses the same guest-managed pattern successfully.

**References:**
- [proxy-wasm ABI specification](https://github.com/proxy-wasm/spec)
- [http-wasm handler ABI](https://http-wasm.io/http-handler-abi/)
- [Component Model memory discussion](https://github.com/WebAssembly/component-model/issues/314)
