...
- **No cancellation signals**: Context cancellation does not reach the guest (except via
  `get_shutdown_requested()` for graceful shutdown)
- **No context values**: Trace IDs, correlation IDs, and other context-attached values are
  not accessible

**Workaround**: For graceful shutdown, receivers SHOULD poll `get_shutdown_requested()`. For
...
**`set_status_reason` is optional:**

The spec uses SHOULD rather than MUST for calling `set_status_reason()` before returning an
...
**Best practice**: Guest implementations SHOULD always call `set_status_reason()` before
...
- Calling `set_result_*()` to forward data to a native exporter
...
validate that required functions (`otelwasm_memory_allocate`, `start`, `shutdown`, etc.) are actually
implemented.
...
|------------------------|--------------------------------------------|-----------------------------------------------|
| `processor.Traces`     | `component.Component + consumer.Traces`    | `start` + `shutdown` + `otelwasm_consume_traces`       |
| `processor.Metrics`    | `component.Component + consumer.Metrics`   | `start` + `shutdown` + `otelwasm_consume_metrics`      |
| `processor.Logs`       | `component.Component + consumer.Logs`      | `start` + `shutdown` + `otelwasm_consume_logs`         |
| `exporter.Traces`      | `component.Component + consumer.Traces`    | `start` + `shutdown` + `otelwasm_consume_traces`       |
| `exporter.Metrics`     | `component.Component + consumer.Metrics`   | `start` + `shutdown` + `otelwasm_consume_metrics`      |
| `exporter.Logs`        | `component.Component + consumer.Logs`      | `start` + `shutdown` + `otelwasm_consume_logs`         |
| `receiver.Traces`      | `component.Component`                      | `start` + `shutdown` + `otelwasm_start_traces_receiver`|
| `receiver.Metrics`     | `component.Component`                      | `start` + `shutdown` + `otelwasm_start_metrics_receiver`|
| `receiver.Logs`        | `component.Component`                      | `start` + `shutdown` + `otelwasm_start_logs_receiver`  |
...
| `factory.CreateTraces(ctx, settings, config, next) (processor.Traces)`  | Module load + `start()` + `otelwasm_consume_traces` |
