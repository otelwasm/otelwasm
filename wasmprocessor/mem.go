...
func marshalLogsIfUnderLimit(mem wazeroapi.Memory, dt plog.Logs, buf uint32, bufLimit bufLimit) int {
	marshaler := plog.ProtoMarshaler{}
	vLen := marshaler.LogsSize(dt)
	if vLen == 0 {
		return 0 // nothing to write
	}
	...
	b, err := marshaler.MarshalLogs(dt)
	if err != nil {
		panic(err) // Bug: in marshaller.
	}
...
func writeBytesIfUnderLimit(mem wazeroapi.Memory, b []byte, buf uint32, bufLimit bufLimit) int {
	vLen := len(b)
	if vLen == 0 {
		return 0 // nothing to write
	}
